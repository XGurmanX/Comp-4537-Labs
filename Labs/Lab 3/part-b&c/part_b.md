...
From inside part-b/:
```bash
docker build -t comp4537-lab3-partb+c .
```

Optional (no cache):
docker build --no-cache -t comp4537-lab3-partb+c .

---

### 3. Re-tag image for Google Artifact Registry
```bash
docker tag comp4537-lab3-partb+c us-west1-docker.pkg.dev/comp-4537-server/lab3/comp4537-lab3-partb+c
```
...
```bash
docker push us-west1-docker.pkg.dev/comp-4537-server/lab3/comp4537-lab3-partb+c
```
...
```bash
gcloud run deploy comp-4537-lab3-partb+c `
  --image us-west1-docker.pkg.dev/comp-4537-server/lab3/comp4537-lab3-partb+c `
  --platform managed `
  --region us-west1 `
...
If changes appear, redeployment succeeded.

---

## One-line Rule

Change code → build → tag → push → deploy
